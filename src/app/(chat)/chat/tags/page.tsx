  ...
  return (
    <section className="h-screen flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className=" p-6 mx-8 w-full">
        ...
        <h2 className="font-bold text-3xl md:text-4xl mt-8 mb-4">New Topic</h2>

        <div className="w-full">
          <input type='text' placeholder="Topic Name" disabled={isSubmitting} className="p-4 w-full border-2 border-gray-200 rounded-xl mb-4" {...register('name')} required/>
          { errors?.name?.message && <div className='text-red-600'>{errors.name?.message}</div> }
        </div>

        <div>
          <input type='text' placeholder="Topic Description" disabled={isSubmitting} className="p-4 w-full border-2 border-gray-200 rounded-xl mb-4" {...register('description')} />
