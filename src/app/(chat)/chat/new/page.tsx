...
export default function NewDiscussion(){
  ...
  const submitPost: SubmitHandler<newPost> = async (data) => {
    if (!editorState || editorState === "<p></p>")
      return notify({ type: "error", message: "No editor text" });
    if (!selectedTags || selectedTags.length === 0)
      return notify({ type: "error", message: "Select a Tag" });
    if (!data.channel || data.channel === "Select Channel")
      return notify({ type: "error", message: "Select a Valid Channel" });
  ...
  return (
    <section className="mx-6 md:mx-12 mt-8 md:mt-12 p-3 md:p-6">
      <h1 className="font-bold text-3xl md:text-4xl mt-8 mb-4">
        Start a Discussion
      </h1>
      ...
      <form onSubmit={handleSubmit(submitPost)}>
        ...
        <div className="flex my-4 gap-4 items-center">
          ...
          <div>
            <select
              ...
              required
            >
              <option defaultValue="Select Channel">
                ---Select a Channel---
              </option>
        ...
        <div className="flex mt-8 gap-6 justify-end">
          <button className="px-8 py-4 bg-zinc-300 hover:bg-zinc-500 rounded-3xl">
            Cancel
          </button>
          <button
            type="submit"
            className="px-8 py-3 bg-zinc-300 hover:bg-zinc-500 hover:text-white rounded-3xl"
          >
